...
const ExperienceForm = ({ data, onChange }) => {
 ...
 const  generateDescription = async(index) => {
   ...
   const prompt = `Enhance this job description  ${experience.description} for the position of ${experience.position} at ${experience.company}. Make it concise (1-2 sentences) and highlight key responsibilities and achievements. Use action verbs and quantifiable results where possible. Make it ATS-friendly.`
   try {
...
generatingIndex === index ?( <Loader2 className='w-3 h-3 animate-spin'/>):( <Sparkles className='w-3 h-3'/> )
                             ...
                             <textarea value={experience.description || ""} onChange={(e)=> updateExperience(index,"description",e.target.value)} className='w-full text-sm px-3 py-2 rounded-lg resize-none ' rows={4} placeholder=' Descirbe your key responsibilities and achievements...'/>
                        </div>
                        </div>
                      ))}
                      </div>
